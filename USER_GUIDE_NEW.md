...
> **1. Family money first.**
> Buffer (cash reserves for survival) is funded before any other allocation. Trading capital, long-term investments, and lifestyle spending all wait their turn behind family protection.

> **2. Trading is a job, not a windfall.**
> Profits flow through a tax reserve and a stage-based waterfall. Money you never see in your spending account is money you cannot impulse-spend.

> **3. The pause is the system.**
> The Spending Gate inserts a moment between impulse and payment. That moment — not willpower, not motivation — is what protects your wealth.
...
|---|---|---|
| ⚠️ **Backup Reminder** | 7+ days since last backup | Click → goes to Rules → click Download as file |
| ⚠️ **Drawdown Alert** | Trading capital is in Caution / Defensive / Stop zone | Click Review → goes to Trading P&L for zone rules |
| ⚠️ **Rollover Prompt** | Last 3 days of month or first 3 days of next month, rollover not yet applied | Click → goes to Budget tab → click Apply rollover |
...
- **Why it matters:** Drives the entire Drawdown Protocol. The high water mark, current drawdown percentage, recovery target, and zone all derive from this number. ⚠️ **If you enter profit instead of total balance, the system thinks your account has grown 10× and your drawdown calculation breaks.**
...
- **Spending Gate** prevents impulse purchases by inserting a 24-hour pause.
- **Quick Log** records purchases you already made.
- **History & Triggers** shows your spending patterns and triggers.
...
4. The gate either:
   - (a) blocks instantly if envelope is in Hard Block mode and would be exceeded,
   - (b) requires PIN if envelope is in PIN Override mode and would be exceeded, or
   - (c) shows the decision screen.
...
- **Why it matters:** ⚠️ Destructive. Take a backup first. Then, if needed, you can re-import after the reset.
...
|---|---|---|
| **Soft warning** | Purchase allowed, warning shown | Essentials (groceries, transport) |
| **Hard block** | Purchase refused | Discretionary (eating out, personal) |
| **PIN override** | Refused unless PIN entered | Safety-valve (emergency, family) |

### Rollover modes (what happens to unspent at month-end)

| Mode | Behavior | Use for |
|---|---|---|
| **Reset** | Unspent lost. Use it or lose it | Groceries |
| **Roll over** | Unspent carries to next month's cap | Lumpy categories (gifts, household repair) |
| **Sweep to buffer** | Unspent transfers directly to your Family Buffer | Personal/eating-out envelopes you want to under-spend |

📌 **Sweep to buffer** is the wealth-building option.

### Strategic combination

Set discretionary envelopes to **Hard Block + Sweep**. The system literally cannot let you over-spend, and rewards under-spending with savings. This converts your daily discipline into automatic wealth-building.
...
⚠️ **Math reality:** down 30% needs +43% to recover. Down 50% needs +100%. Down 70% needs +233%. The protocol changes behavior automatically before the math becomes catastrophic.
...
⚠️ **Where to keep backups:** Your Downloads folder isn't enough. Email backups to yourself, save to Google Drive, or copy to a USB. Multiple copies, multiple locations.

### Restore process

Rules → Import backup → select a JSON → confirms → loads. All current data is replaced.
...
- [ ] Open the app.
- [ ] Glance at Command tab — Net Worth, Stage, Months Stored.
- [ ] Read any warning banners. Act on them or note them.
- [ ] Close the app.
...
- [ ] Fill: What is it? + Amount + Category + Envelope + Trigger.
- [ ] Click Run through the gate.
- [ ] Read Hours of work / Left this month / 30-year value.
- [ ] Click Skip / Sleep / Buy.
...
- [ ] Fill: What did you buy? + Amount + Category + Trigger.
- [ ] Click Log it.
...
- [ ] Command — overall state.
- [ ] Budget — envelope pace, anything trending over.
- [ ] Impulse Control → History & Triggers — what triggered me this week.
- [ ] Adjust if needed (tighten an envelope, talk with wife about a pattern).

---

## 🗓️ Month-end ritual (15 minutes, last day of month)

- [ ] Open Review tab (or click the auto-popup).
- [ ] Read all four metric cards.
- [ ] Read envelope performance.
- [ ] Click Log P&L action → enter month's net P&L in Trading P&L.
- [ ] Click Apply rollover action → applies sweep in Budget.
- [ ] Click Take snapshot action → captures state and downloads backup.
- [ ] Click Mark month as reviewed.
- [ ] Email the backup file to yourself (or save to Google Drive).
...
### ⚠️ 1. Entering profit instead of capital in Trading P&L → Capital field
...
### ⚠️ 2. Not updating trading capital regularly
...
### ⚠️ 3. Setting unrealistic spending budgets
...
### ⚠️ 4. Bypassing the Spending Gate ("just this once")
...
### ⚠️ 5. Setting all envelopes to Soft warning
...
### ⚠️ 6. Setting all envelopes to Reset rollover
...
### ⚠️ 7. Forgetting to Apply rollover
...
### ⚠️ 10. Resetting High Water mark to "fix" a drawdown
...
> 1. **Family Buffer**
> 2. **Months Stored**
> 3. **This month's spending vs budget**

---

> ## 🔒 The 1 rule you never break
>
