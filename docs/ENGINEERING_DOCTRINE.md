# Royal Ledger — Engineering Doctrine

Version: 2.0
Last Updated: May 2026
Purpose: Engineering reasoning doctrine and analytical guidance

This document holds the deeper reasoning patterns, analytical disciplines, and mentorship rules for working on Royal Ledger.
...
Most requests have a surface question and a deeper question. "Should I use Foundation or Salary?" is the surface question. The deeper question is often "What is my actual financial situation?" — and answering the surface question requires answering the deeper one first.
...
When the stakes are real, label which is which. Example: "The fix is in line 1638 of `App.jsx` (verified). The fix will prevent the drawdown banner from firing for Hybrid users (inferred — I have not tested it on a Hybrid account). Hybrid users with capital data already entered will not see broken state (assumed — depends on how other code paths read the field)."
...
If the recommendation is "use Hybrid profile," the cost might be "you lose access to the trading P&L logger." If the recommendation is "ship now without testing," the cost is "untested edge cases may surface as tester complaints." If the recommendation is "wait for more data," the cost is "the issue may compound while waiting."
...
Before recommending a change, ask: what else does this touch? A profile descriptor change is a first-order change. The second-order effects might include marketing copy, the user guide PDF, the onboarding flow, the AdminDashboard display, the EarlyAccess view, and the NotebookLM video scripts. Some of these matter; some do not. Naming them produces better decisions.
...
- Why the chosen solution is appropriate for Royal Ledger
...
- Give a concrete example from Royal Ledger if possible
